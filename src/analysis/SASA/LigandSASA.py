...
class LigandSASACalculator:
    def __init__(self, complex_path: str, ligand_resname: str):
    ...
    def calculate_ligand_sasa(self) -> float:
        try:
            structure, sasa_result = self._get_sasa(self.complex_path)
            # リガンドのSASAを選択して計算する
            selection = freesasa.selectArea([f"ligand, resn {self.ligand_resname}"], structure, sasa_result)
            ligand_sasa = selection['ligand']
            return ligand_sasa
        except Exception as e:
