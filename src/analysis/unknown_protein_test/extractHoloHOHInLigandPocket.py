...
def main():
    ...
    ligand_pocket_definer = LigandPocketDefiner(pdb_name=pdb_name, voxel_num=ligand_voxel_num)

    ligand_pocket = ligand_pocket_definer.define_ligand_pocket()[0]
    print(ligand_pocket.sum())

    # input_pdb_path = f"/mnt/ito/test/{protein_name}/holo/{holo_name}/{holo_name}_HOH.pdb"
    input_pdb_path = 'HOH.pdb'
    # output_pdb_path = f"/mnt/ito/test/{protein_name}/holo/{holo_name}/{holo_name}_{apo_name}_HOH_in_ligand.pdb"
    output_pdb_path = f'{pdb_name}_HOH_in_ligand_pocket.pdb'
    ...
    grid_dims, grid_origin = get_voxel_info(pdb_name=pdb_name)
