...
def get_ndarray(pdb_list, base_path, gr_voxel_num):
    data_list = []
    for pdb_name in pdb_list:
        ...
        for data_name in data_path_list:
            ...
            loaded_data_reshaped = loaded_data.reshape((1, gr_voxel_num*2+1, gr_voxel_num*2+1, gr_voxel_num*2+1, 1))
...
def get_data(pdb_list, base_path, gr_voxel_num):
    displaceable_data = get_ndarray(pdb_list, os.path.join(base_path, 'displaceable/'), gr_voxel_num)
    displaceable_labels = np.ones(len(displaceable_data))

    non_displaceable_data = get_ndarray(pdb_list, os.path.join(base_path, 'non_displaceable/'), gr_voxel_num)
...
def load_data(pdb_list_path, ligand_voxel_num, gr_voxel_num):
    with open(pdb_list_path, 'r') as f:
        pdb_list = f.read().splitlines()
        base_path = f'/home/ito/research/data/LIGAND_POCKET_VOXEL_NUM_{ligand_voxel_num}/gr_data/GR_VOXEL_NUM_{gr_voxel_num}/'
        data, labels = get_data(pdb_list, base_path, gr_voxel_num)
    return data, labels
